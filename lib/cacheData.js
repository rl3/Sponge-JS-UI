...
var join= function( /*args*/ ) {
    ...
    return args.map(function( a ) { return _.isArray(a) ? a.map(oid2Str).join('-') : oid2Str(a); }).join('/');
};
...
var getUserKeyPrefix= function( noAuth ) {
    if ( noAuth ) return '';
...
var buildCacheKey= function( urlData, noAuth ) {
    ...
    return getUserKeyPrefix(urlData.noAuth) + keys.join(':');
};
...
SpongeTools.getUserKeyPrefix= getUserKeyPrefix;
